...
import { MapPin, CheckCircle, Info, ArrowLeft } from "lucide-react";
    ...
    <div>
      ...
      {!isLoadingLeads && Object.entries(grouped).map(([city, cityTargets], cityIndex) => (
        <div key={city} className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            <MapPin className={cityTargets[0]?.source === 'dynamic' ? 'text-red-500' : 'text-accent'} />
            ...
            {cityTargets.map((target, i) => {
              ...
              return (
                ...
                >
                  ...
                  {isContacted && <CheckCircle className="absolute top-4 right-4 text-green-500" />}

                  <div className={isDynamic ? 'mt-7' : ''}>
                    <div className="flex items-center gap-2.5 mb-1">
                      {isDynamic ? <User className="text-red-500 flex-shrink-0" /> : <Zap className="text-accent flex-shrink-0" />}
                      <h3 className="text-xl font-black">{target.name}</h3>
                    </div>
                    <p className="text-sm opacity-50 mb-6 flex items-center gap-1.5">
                      <Phone size={12} /> {target.description}
                    </p>
