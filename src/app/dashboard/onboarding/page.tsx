  ...
  const handleCreateAgent = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("bots").insert([{
        name: t(`Agente ${industry}`, `${industry} Agent`),
        description: t(`Agente IA para ${purpose} en ${industry}`, `AI agent for ${purpose} in ${industry}`),
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h2 className="mb-2 font-serif text-2xl text-text-primary">
                {t("¿Cuál es tu industria?", "What's your industry?")}
              </h2>
              <p className="mb-8 text-sm text-text-secondary">
                {t("Esto ayuda a personalizar tu agente IA", "This helps personalize your AI agent")}
              </p>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h2 className="mb-2 font-serif text-2xl text-text-primary">
                {t("¿Qué necesitas automatizar?", "What do you need to automate?")}
              </h2>
              <p className="mb-8 text-sm text-text-secondary">
                {t("Elige el propósito principal de tu agente", "Choose your agent's main purpose")}
              </p>
              <div className="flex flex-col gap-3">
                {PURPOSES.map((p) => (
                  ...
                  >
                    <div className="font-medium text-text-primary">{t(p.es, p.en)}</div>
                    <div className="mt-0.5 text-xs text-text-secondary">{t(p.desc_es, p.desc_en)}</div>
                  </button>
                ))}
              </div>
            </motion.div>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h2 className="mb-2 font-serif text-2xl text-text-primary">
                {t("Conecta tu WhatsApp", "Connect your WhatsApp")}
              </h2>
              <p className="mb-8 text-sm text-text-secondary">
                {t("Opcional — puedes configurarlo después", "Optional — you can set it up later")}
              </p>
              <input
                type="tel"
                value={whatsapp}
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="text-center">
              ...
              <h2 className="mb-2 font-serif text-2xl text-text-primary">
                {t("¡Tu agente IA está activo!", "Your AI agent is live!")}
              </h2>
              <p className="mb-8 text-sm text-text-secondary">
                {t(
              ...
              >
                {loading
                  ? t("Creando agente...", "Creating agent...")
