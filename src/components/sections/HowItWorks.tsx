  ...
  return (
    <section id="como-funciona" style={{ padding: 'clamp(4rem, 10vw, 8rem) clamp(1.5rem, 5vw, 4rem)', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', letterSpacing: '0.12em', color: '#D4AF37', textTransform: 'uppercase' }}>{t.how.label}</span>
      </div>
      <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 5vw, 3.5rem)', lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: '16px', color: '#f0f2f8' }}>{t.how.title}<br /><em style={{ color: '#D4AF37', fontStyle: 'italic' }}>{t.how.titleEm}</em></h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)' }}>
        {steps.map((step: any, i: number) => (
          <motion.div key={step.num} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} style={{ padding: '32px', background: '#0d1017', borderRight: i < steps.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none', position: 'relative' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#4a5568', letterSpacing: '0.08em', marginBottom: '20px' }}>{step.num} —</div>
            <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.3rem', marginBottom: '12px', color: '#f0f2f8' }}>{step.title}</h3>
