...
export default function AIPlayground() {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Simulamos respuesta rápida o usamos la API real si está disponible
      // Para demo, usamos un mock inteligente para que se sienta rápido
      ...
      setMessages(prev => [...prev, { role: "ai", text: data.response || "Hola, ¿en qué puedo ayudarte?" }]);
    } catch (e) {
      // Fallback si la API falla
      setMessages(prev => [...prev, { role: "ai", text: "¡Hola! Soy Nova. Prueba preguntándome sobre precios o cómo automatizar tus ventas." }]);
    } finally {
      setIsTyping(false);
    }
  };
          ...
          >
            ...
            >
              ...
              <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ color: '#D4AF37', margin: 0, fontSize: '16px', fontWeight: 700 }}>Nova · Agente IA</h3>
                  <p style={{ color: '#27C93F', margin: 0, fontSize: '12px' }}>En línea • Demo en vivo</p>
                </div>
                <button onClick={() => setIsOpen(false)} style={{ background: 'none', border: 'none', color: '#8892a4', fontSize: '24px', cursor: 'pointer' }}>×</button>
              </div>

              {/* Messages Area */}
              <div style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {messages.length === 0 && (
                  <div style={{ textAlign: 'center', color: '#4a5568', fontSize: '13px', marginTop: '40px' }}>
                    👋 Hola, soy Nova. Pregúntame sobre precios, características o pídele una demo a mi creador.
                  </div>
                )}
                {messages.map((msg, i) => (
